...
import { ConfigTablesProvider } from '../config-tables/config-tables';
import { AppUtilsProvider } from '../app-utils/app-utils';
import { ApiHelperProvider } from '../api-helper/api-helper';
import { ProductsProvider } from '../products/products';
import { Events, App } from 'ionic-angular';
import { Toast, AlertController } from 'ionic-angular';
...
/*
  Generated class for the SharedhelperProvider provider.

  See https://angular.io/guide/dependency-injection for more info on providers
  and Angular DI.
*/
@Injectable()
export class SharedhelperProvider {
  ...
  register_data = { CompanyName: '', UserName: '', FirstName: '', LastName: '', PhoneNumber: '', Email: '', City: '', Password: '', Reseller: '' };
  searchTerm: string = '';
  searchitems: any;
  searching: any = false;
  ...
  constructor(public sessionProvider: SessionHelperProvider, public configProvider: ConfigTablesProvider,public events: Events,
     public appUtils: AppUtilsProvider, public apiHelper: ApiHelperProvider,public appCtrl: App,
    public alertCtrl: AlertController, public productsProvider: ProductsProvider) {
    console.log('Hello SharedhelperProvider Provider');
    ...
    this.FirstLevelCategories = [] ;
     ...
     this.configProvider.GetFirstLevelCategories().subscribe(res => {
       ...
       if (res.rows.length == 0) {
         ...
         this.sessionProvider.GetValuesFromSession('LastSync').then((SyncDate) => {
            this.LastSync = SyncDate ;
         });
  ...
  Register() {
    ...
    if (this.register_data.FirstName == '') {
      ...
      this.apiHelper.RequestPostHttp(this.register_data, 'Register' , true).then(result => {
        ...
        if (result.Status == false) {
          ...
          //   this.sessionProvider.SaveValueInSession('userEmail' , this.register_data.FirstName + ' ' + this.register_data.LastName);
          // }else{
          //   this.sessionProvider.SaveValueInSession('userEmail' , this.register_data.Email);
          // }
          this.sessionProvider.SaveValueInSession('userEmail' , this.register_data.Email);
          this.register_data.City = '' ;
          this.register_data.CompanyName = '' ;
          this.register_data.Email = '' ;
          this.register_data.FirstName = '' ;
          this.register_data.LastName = '' ;
          this.register_data.Password = '' ;
          this.register_data.PhoneNumber = '' ;
          this.register_data.Reseller = '' ;
